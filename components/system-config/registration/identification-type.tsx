...
import { identificationTypesAPI, IdentificationType } from "@/lib/api/reference"
import { toast } from "sonner"
import { format } from "date-fns"

export default function IdentificationTypePage() {
  const [data, setData] = useState<IdentificationType[]>([])
  const [filteredData, setFilteredData] = useState<IdentificationType[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [searchQuery, setSearchQuery] = useState("")
  const [editModalOpen, setEditModalOpen] = useState(false)
  const [deleteModalOpen, setDeleteModalOpen] = useState(false)
  const [editItem, setEditItem] = useState<IdentificationType | null>(null)
  const [deleteItem, setDeleteItem] = useState<IdentificationType | null>(null)
  ...
  const fetchData = async () => {
    setIsLoading(true)
    try {
      const result = await identificationTypesAPI.getAll()
      setData(result); setFilteredData(result)
    } catch { toast.error("Failed to load identification types") } finally { setIsLoading(false) }
  }
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `identification-types-${Date.now()}.csv`; a.click()
  }
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-800">Identification Types ({filteredData.length})</h2>
      ...
      <div className="relative w-72 mb-6">
        ...
        <Input placeholder="Search Identification Types" className="pl-10 bg-gray-50" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
      </div>
      <Table>
        ...
        <TableBody>
          {filteredData.length === 0 ? <TableRow><TableCell colSpan={7} className="text-center text-gray-400 py-8">No identification types found</TableCell></TableRow> :
...
function AddEntityModal({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await identificationTypesAPI.create({ name: name.trim(), code: code.trim().toUpperCase(), description: description.trim() || undefined, isActive })
      toast.success("Identification type added")
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] py-5">
        <DialogHeader><DialogTitle className="text-lg font-bold">Add Identification Type</DialogTitle></DialogHeader>
        <div className="space-y-4 py-4">
          <div><label className="text-sm font-medium mb-2 block">Identification Type Name <span className="text-red-500">*</span></label><Input placeholder="Enter name (e.g., NIN)" value={name} onChange={(e) => setName(e.target.value)} disabled={isSubmitting} /></div>
          <div><label className="text-sm font-medium mb-2 block">Code <span className="text-red-500">*</span></label><Input placeholder="Enter code (e.g., NIN)" value={code} onChange={(e) => setCode(e.target.value)} disabled={isSubmitting} /></div>
          ...
          <div className="flex items-center gap-2"><input type="checkbox" id="isActiveID" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} disabled={isSubmitting} className="w-4 h-4" /><label htmlFor="isActiveID" className="text-sm font-medium">Is Active</label></div>
        </div>
        <DialogFooter className="flex justify-end gap-2">
          ...
          <Button className="bg-green-800 hover:bg-green-900" onClick={handleSubmit} disabled={isSubmitting}>
            {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Adding...</> : "Add Identification Type"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function EditModal({ open, onClose, item, onSuccess }: { open: boolean; onClose: () => void; item: IdentificationType | null; onSuccess: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setIsSubmitting(true)
    try {
      await identificationTypesAPI.update(item.id, { name: name.trim(), code: code.trim().toUpperCase(), description: description.trim() || undefined, isActive })
      toast.success("Identification type updated")
      onClose(); onSuccess()
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] py-5">
        <DialogHeader><DialogTitle className="text-lg font-bold">Edit Identification Type</DialogTitle></DialogHeader>
        <div className="space-y-4 py-4">
          <div><label className="text-sm font-medium mb-2 block">Identification Type Name <span className="text-red-500">*</span></label><Input placeholder="Enter name" value={name} onChange={(e) => setName(e.target.value)} disabled={isSubmitting} /></div>
          ...
          <div><label className="text-sm font-medium mb-2 block">Description</label><Textarea placeholder="Enter description" value={description} onChange={(e) => setDescription(e.target.value)} disabled={isSubmitting} rows={3} /></div>
          <div className="flex items-center gap-2"><input type="checkbox" id="editIsActiveID" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} disabled={isSubmitting} className="w-4 h-4" /><label htmlFor="editIsActiveID" className="text-sm font-medium">Is Active</label></div>
        </div>
        <DialogFooter className="flex justify-end gap-2">
...
function DeleteModal({ open, onClose, item, onSuccess }: { open: boolean; onClose: () => void; item: IdentificationType | null; onSuccess: () => void }) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true)
    try {
      await identificationTypesAPI.delete(item.id)
      toast.success("Identification type deleted")
      onClose(); onSuccess()
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[400px] py-5">
        <DialogHeader><DialogTitle className="text-lg font-bold">Delete Identification Type</DialogTitle></DialogHeader>
        <div className="py-4">
